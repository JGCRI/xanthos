...
class HydropowerActual:
    """
    Computes country and GCAM-region level hydropower production time series based on xanthos streamflow otuput.
    ...
    def get_grid_id(self, longlat):
        """
        Gets the grid location of a dam based on longitude/latitude.
        """
    ...
    def get_drain_area(self, x, drainage_area):
        """
        Gets drainage area implied by the routing network.
        """
    ...
    def get_power(self, res):
        """
        MAIN SIMULATION FUNCTION... NEXT VERSION TO INCLUDE ev, area, max_depth, installed_cap
        """
    ...
    def sim_vars(self, idx, res):
        ...
        if math.isnan(self.cap_live) == True:
    ...
    def to_region(self):
        ...
        Convert power production time series to GCAM region enery production.
        """
        power_all_dams_monthly = pd.DataFrame(self.power_all_dams).set_index(pd.period_range(self.start_date, periods=len(self.power_all_dams), freq="M"))
        energy_all_dams = power_all_dams_monthly.resample("A").mean() * HydropowerActual.hours_in_year * HydropowerActual.mwh_to_exajoule
