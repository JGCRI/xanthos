...
def HydropowerPotential(settings, q_grids):
    ...
    e_grids = q_ex_h * hyd_grid_data["elevD"][:,
                       np.newaxis]  # Multiply by elevation difference (head) to get energy (TWh)
    e_grids_ = pd.DataFrame(e_grids.T)
    e_grids__ = e_grids_.set_index(
        pd.period_range(settings.hpot_start_date, periods=len(e_grids_), freq="M"))  # Time-stamp the data
    e_grids_annual = e_grids__.resample(
        "A").sum() * twh_to_exajoule  # Sum to annual energy production, then convert to EJ
    ...
    filename_techpot = os.path.join(settings.OutputFolder,
                                    "tech_hydro_pot_by_gcam_region_EJ_{}.csv".format(settings.ProjectName))

    pd.DataFrame.to_csv(techpot_.T, filename_techpot)
    ...
    filename_techpot_expl = os.path.join(settings.OutputFolder,
                                         "tech_expliot_hyd_pot_by_gcam_region_EJ_{}.csv".format(settings.ProjectName))

    pd.DataFrame.to_csv(techpot_expl_.T, filename_techpot_expl)
