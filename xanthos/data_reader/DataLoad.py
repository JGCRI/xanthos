...
def load_gcm_data(settings):
    ...
    pre = load_const_griddata(settings.PrecipitationFile, 0, settings.PrecipVarName)

    # temperature in degrees C
    temp = load_const_griddata(settings.TemperatureFile, 0, settings.TempVarName)

    # daily temperature range in degrees C
    dtr = load_const_griddata(settings.DailyTemperatureRangeFile, 0, settings.DTRVarName)
...
def check_size(data, ng, nm):
    ...
    for d in data:
        if not d.shape[0] == ng:
            print("Error: Inconsistent {0} data grid size. Expecting size: {1}. Received size: {2}".format(var[m], ng, d.shape[0]))
            n += 1
        if not d.shape[1] == nm:
            print("Error: Inconsistent {0} data grid size. Expecting size: {1}. Received size: {2}".format(var[m], nm, d.shape[0]))
            n += 1
        m += 1
