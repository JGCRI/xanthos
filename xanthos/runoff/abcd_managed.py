...
class AbcdManaged:
    ...
    def abcd_dist(self, i, pet, tmin):
        ...
        if not self.nosnow:
            ...
            self.snm[i, allrain] = self.snowpack[i, allrain] * self.m
            self.snm[i, rainorsnow] = (self.snowpack[i, rainorsnow] * self.m) * \
        ...
        self.et_op[i, :] = rpt_over_pt2 - np.sqrt(np.round(np.square(rpt_over_pt2) - (self.water_avail[i, :] * self.b_over_a), 4))
